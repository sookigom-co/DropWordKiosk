...
describe('areaFilled (SOO-1049 후속 4/5 채움 중단)', () => {
  ...
  it('FILL_STOP_RATIO 는 0.80(SOO-1208 후속 — 보더 요청 "80%까지 채워줘", 원 패킹 이론 상한 아래)', () => {
    // 원은 겹치지 않고 면적의 ~90%(육각 패킹 이론값)를 넘길 수 없다. 0.80 은 그 상한 아래이면서
    // 이전 0.70 보다 빽빽하게 채운다. 스폰은 firstFreeSpawn 비중첩 자리에서만 이뤄져 겹침은 없다.
    expect(FILL_STOP_RATIO).toBe(0.80);
  });
});
