...
describe('areaFilled (SOO-1049 후속 4/5 채움 중단)', () => {
  ...
  it('FILL_STOP_RATIO 는 0.70(SOO-1208 후속 — 보더 요청 "70%로 재조절", 원 패킹 이론 상한 아래)', () => {
    // 원은 겹치지 않고 면적의 ~90%(육각 패킹 이론값)를 넘길 수 없다. 0.70 은 그 상한 아래이면서
    // 0.75 보다 약간 여유 있는 밀도다. 스폰은 firstFreeSpawn 비중첩 자리에서만 이뤄져 겹침은 없다.
    expect(FILL_STOP_RATIO).toBe(0.70);
  });
});
