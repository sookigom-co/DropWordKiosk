 ...
 * 100% 패킹은 빈틈이 사라져 신규 버블이 이웃에 바짝 붙어 태어나고(성장 여유 부족) 물리
 * 솔버가 미세하게 밀어내며 "부르르 떨림"을 유발한다는 보더 피드백에 따라 0.9 로 낮춘다.
 * 면적의 10% 를 여유로 남겨 스폰 자유 공간 탐색(firstFreeSpawn, 목표 반지름 기준)이
 * 항상 빈자리를 찾을 여지를 주고, 겹친 채 태어나는 스폰을 구조적으로 방지한다.
 * (비중첩 원들의 총 면적은 필드 면적을 넘을 수 없으므로 이 값은 안전한 상한 역할.)
 */
export const FILL_STOP_RATIO = 0.9;
