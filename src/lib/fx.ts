 ...
 * 쌓인 버블은 영속).
 *
 * SOO-1208 후속(보더 요청 2026-08-29): "75%까지 올려" → 0.65 → 0.75 → 0.70 →
 * **0.80**("80%까지 채워줘", PR #91 머지 후 재조절). 이론 상한(~90%) 아래이면서
 * 세 쌍 비겹침(단어↔단어·단어↔보라·보라↔보라)을 유지할 수 있는 채움 밀도.
 */
export const FILL_STOP_RATIO = 0.80;
 ...
 * 낙하 버블이 모두 정착"한 시점(= 좌표 안정) 을 채움 완료로 보고, 그로부터 이 유예 뒤 고정한다.
 *
 * SOO-1208 후속(보더 요청 2026-08-29): "좌표가 안정되면 2초 뒤에 멈추는 걸로" → 1초 → **2초**.
 * fillCompleteAt(좌표 안정 래치) 시점부터 이 유예가 지나면 전체 정지한다.
 */
