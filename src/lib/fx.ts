 ...
 * 보더 요청 변천: "2/3" → "4/5" → "가득 채움(1.0)" → "0.9(90%)" → **"0.6(60%)"**(현재, SOO-1112).
 *
 * 왜 90% 가 아니라 60% 인가(겹침 방지의 기하학적 근거):
 * 이 값은 "원 면적의 합 ÷ 필드 면적"의 상한이다. 원(圓)은 아무리 잘 배치해도 서로
 * 겹치지 않고는 면적의 ~90%(완벽 육각 패킹의 이론값)를 넘길 수 없고, 물리 솔버·부력으로
 * 눌리는 실제 상황에서는 그보다 훨씬 낮은 밀도에서 이미 겹치기 시작한다. 즉 목표를 90% 로
 * 두면 버블끼리 겹치는 것은 기하학적으로 불가피했다(SOO-1112 보더 스크린샷의 겹침 원인).
 * 60% 로 낮춰 버블 사이에 실질적 여유를 남기면, 부력으로 눌려도 솔버가 서로 떼어 놓을
 * 공간이 생겨 눈에 보이는 겹침이 사라진다. 대신 화면은 "가득"보다 덜 빽빽하게 채워진다
 * (겹침 0 ↔ 빽빽한 채움은 상충 관계 — 밀도는 보더 판단으로 조정).
 */
export const FILL_STOP_RATIO = 0.6;
