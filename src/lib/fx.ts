 ...
 * 버블 겹침이 기하학적으로 불가피해 60% 까지 낮췄다가 보더 요청으로 80% 로 올렸으나,
 * 다시 "70% 로 줄여 달라" 요청(SOO-1112 최신). 70% 는 이론 상한(~90%) 아래이면서 80%
 * 보다 여유 있게 채운다. 스폰은 여전히 `firstFreeSpawn` 기준 비중첩 자리에서만 이뤄지므로
 * 스폰 순간 겹침은 없고, 채움만 조절된다(자유 공간이 없으면 그 틱은 보류).
 */
export const FILL_STOP_RATIO = 0.7;
